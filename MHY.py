...
class InputTools:
	@staticmethod
	def simple_select(data_type, prompt: str, recursive: bool = True, response: str = None) -> str:
		try:
			return data_type(input(prompt))
		except ValueError:
			print(response)
			if recursive:
				return InputTools.simple_select(data_type=data_type, prompt=prompt, recursive=recursive, response=response)
...
class ApiParser:
	...
	def select_game(self) -> str:
		...
		selected_game: int = InputTools.simple_select(data_type=int, prompt='Select game: ', recursive=True, response='Invalid input! Please enter a valid number.') - 1
	...
	def get_game_main(self, game_index: int) -> dict: # main or pre_download
		if self.is_pre_download(game_index) and InputTools.simple_yn(prompt='Pre-download available. Pre-download? (y/N) '):
	...
	def get_game_patches(self, game_main: dict) -> dict:
		...
		previous_ver: int = InputTools.simple_select(data_type=int, prompt='Select version: ', recursive=True, response='Invalid input! Please enter a valid number.')
	...
	def get_game_pkgs(self, game_major: dict, types: list = ['game_pkgs', 'audio_pkgs'], languages: list[str, ...] = ['en-us'], print_info: bool = False) -> list[tuple[str, int, str], ...]:
		pkgs: list[dict] = [game_major.get(t) for t in types if game_major.get(t)]
		...
		for pkg in pkgs:
			for pkg_info in pkg:
				...
				if 'language' in pkg_info and pkg_info['language'] not in languages:
					continue
	...
	def main(self, patches: bool = False, types: list = ['game_pkgs', 'audio_pkgs'], languages: list[str, ...] = ['en-us'], print_info: bool = False) -> list[tuple[str, int, str], ...]:
		...
		game_main: dict = self.get_game_main(game_index)

		if patches:
			game_major: dict = self.get_game_patches(game_main)
		else:
			game_major: dict = self.get_game_major(game_main)
...
class ArgsHandler:
	def __init__(self):
		...
		parser.add_argument('-o', '--old', action='store_true', help='download patches for older versions', required=False)
		...
		parser.add_argument('-p', '--print', action='store_true', help='print out information instead of downloading', required=False)
		parser.add_argument('--path', type=str, help='download folder path', required=False)
	...
	def listener(self):
		...
		if self.args.path:
			self.args.path = os.path.normpath(self.args.path)
		else:
			self.args.path = ''
